    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                // Get 8 featured products (mix of all types)
                const allProducts = response.data || [];
                const featuredProducts = allProducts
                  .filter(p => p.isFeatured || p.discountPercentage > 0)
                  .slice(0, 8);
                setProducts(featuredProducts.length > 0 ? featuredProducts : allProducts.slice(0, 8));
            } catch (err) {
