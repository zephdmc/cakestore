...
export default function ProductCard({ product, viewMode = 'grid' }) {
    ...
    return (
        ...
        >
                ...
                <div className="absolute top-3 left-3 space-y-2">
                    ...
                    {product.isCustom && (
                        <motion.span
                            ...
                            className="bg-gradient-to-r from-orange-500 to-red-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg block"
                        >
                            Custom Design
                        </motion.span>
                    )}
            ...
            {/* Content Container */}
            <div className={`p-5 flex-1 ${isListView ? 'flex flex-col justify-between' : ''}`}>
                <div>
                    {/* Product Name */}
                    <Link to={`/products/${product.id}`}>
                        <h3 className={`font-bold text-gray-900 mb-2 hover:text-blue-600 transition-colors duration-200 group-hover:underline ${
                            isListView ? 'text-xl' : 'text-lg line-clamp-2'
                        }`}>
                            {product.name}
                        </h3>
                    </Link>
                    ...
                    <div className="flex items-center text-sm text-gray-600 mb-3 space-x-4">
                        {product.capacity && (
                            <div className="flex items-center">
                                <FiDroplet size={14} className="mr-1 text-blue-500" />
                        ...
                        {product.materials && product.materials.length > 0 && (
                            <div className="flex items-center">
                                <FiPackage size={14} className="mr-1 text-blue-500" />
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-4">
                            {product.designTags.slice(0, 3).map((tag, index) => (
                                <motion.span
                                    ...
                                    className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full font-medium flex items-center"
                                >
                    ...
                    {product.features && product.features.length > 0 && (
                        <div className="flex flex-wrap gap-1 mb-4">
                            {product.features.slice(0, 2).map((feature, index) => (
                                <span
                                    key={index}
                                    className="text-xs bg-teal-100 text-teal-700 px-2 py-1 rounded-full font-medium flex items-center"
                                >
                ...
                <div className={`flex items-center justify-between mt-auto ${
                    isListView ? 'pt-4 border-t border-gray-100' : ''
                }`}>
                    {/* Price Display */}
